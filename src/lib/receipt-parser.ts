...
const TOTAL_PATTERNS = [
  ...
  new RegExp(String.raw`(?:\d+\s+)?BALANCE\s+DUE\s+${OCR_CURRENCY}(?<total>${OCR_MONEY_VALUE})`, "i"),
];
...
const OCR_PRICE_PATTERN = /[$§£](\d+(?:[.,]\s*\d{2})?)/g;
const NON_ITEM_PATTERN = /^(?:ACNT|ACCT|ACCOUNT|DATE|GST|OPERATOR|PHONE|REC(?:EIPT)?|STICKY\s+CLUB|TAX\s+INVOICE)\b/i;
...
function parseItemLine(line: string): ReceiptItem | null {
  ...
  if (prices.length >= 2 && prices[0].index !== undefined) {
    ...
    return {
      ...
      quantity: parseOcrQuantity(prefixMatch.groups.quantity),
      unitPrice: parseOcrMoney(prices[0][1]),
      amount: parseOcrMoney(prices.at(-1)?.[1] ?? "0"),
    };
  }
  ...
  const amount = parseOcrMoney(legacyMatch.groups.amount);
  return {
...
function parseItemLines(lines: string[]) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    const isContinuation = index > 0
      ...
      && /^\S{1,3}$/.test(prefix);
    const item = isContinuation ? parseItemLine(`${lines[index - 1]} ${line}`) : parseItemLine(line);
...
export function parseReceiptText(rawText: string): ParsedReceipt {
  ...
  const items = parseItemLines(lines.slice(0, total.index));
